...
class ch_window(QtWidgets.QMainWindow):
    ...
    def Setting(self):
        ...
        for KZ_txt in glob.glob("*.txt"):
            open_txt = open(path+"/OutputKZ/" + KZ_txt, "r")
        ...
        for s_file in glob.glob("Setting.txt"):
            if s_file == "Setting.txt":
                setting_file = open(path + "/Setting.txt", "r")
    ...
    def YES_button(self):
        ...
        if step >len(KZ_string):
            ...
            setting_file = open(path+"/Setting.txt","w+")
            setting_file.write(str(step))
            setting_file.close()
            final_txt_kz = open(path+"/Final/Final_KZ.txt","w+")
            for x in final_kz:
    ...
    def NO_button(self):
        ...
        if step > len(KZ_string):
            ...
            setting_file.write(str(step))
            setting_file.close()
            final_txt_not = open(path+"/Final/Final_NOT.txt","w+")
            for x in final_not:
